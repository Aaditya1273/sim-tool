...
const FRAX_FINANCE_POOLS = "https://api.frax.finance"; // Mock for demo
...
export const simulateHarvest = createTool({
	...
	fn: async ({ poolIds, investmentAmount, durationDays, autoCompound }) => {
		...
		const avgApy = 12; // Mock average APY
		const gasPerTransaction = 5; // Mock gas cost in USD
		...
		return {
			...
			disclaimer:
				"This is a simulation based on current market conditions. Actual returns may vary.",
		};
	},
});
...
export const getFraxPools = createTool({
	...
	fn: async ({ includeKRWQ }) => {
		try {
			...
			return {
				error: "Failed to fetch Frax pools",
				message: error.message,
				mockData: {
					sponsor: "Frax Finance - Hackathon Sponsor",
					fraxPools: [
	...
	fn: async () => {
		try {
			...
			return {
				...
				faucet: {
					url: getFaucetUrl(),
					description: "Get free frxETH test tokens for development",
				},
